...
# this is https://github.com/rails/rails/blob/4-2-stable/activesupport/test/caching_test.rb
# we want to run it after encrypted_store is loaded to make sure we don't break
# any rails cache behavior
...
class FileStoreTest < ActiveSupport::TestCase
  ...
  # this test fails for me with or without encrypted_store loaded
...
class WithEncryption
  class FileStoreTest < ::FileStoreTest
    def setup
      Dir.mkdir(cache_dir) unless File.exist?(cache_dir)
      @secret = SecureRandom.hex(64)
      @cache = ActiveSupport::Cache.lookup_store(:file_store, cache_dir, :expires_in => 60, encrypt: true, encryption_key: @secret)
      @peek = ActiveSupport::Cache.lookup_store(:file_store, cache_dir, :expires_in => 60, encrypt: true, encryption_key: @secret)
      @cache_with_pathname = ActiveSupport::Cache.lookup_store(:file_store, ::Pathname.new(cache_dir), :expires_in => 60, encrypt: true, encryption_key: @secret)
  ...
  class MemoryStoreTest < ::MemoryStoreTest
    def setup
      @secret = SecureRandom.hex(64)
      @record_size = ActiveSupport::Cache.lookup_store(:memory_store,  encryption_key: @secret).send(:cached_size, 1, ActiveSupport::Cache::Entry.new("aaaaaaaaaa", encrypt: true))
      @cache = ActiveSupport::Cache.lookup_store(:memory_store, :expires_in => 60, :size => @record_size * 10 + 1, encrypt: true, encryption_key: @secret)
    end
  end


  class MemCacheStoreTest < ::MemCacheStoreTest
    def setup
      skip "memcache server is not up" unless MEMCACHE_UP
      @secret = SecureRandom.hex(64)
      @cache = ActiveSupport::Cache.lookup_store(:mem_cache_store, :expires_in => 60, encrypt: true, encryption_key: @secret)
      @peek = ActiveSupport::Cache.lookup_store(:mem_cache_store, encrypt: true, encryption_key: @secret)
      @data = @cache.instance_variable_get(:@data)
      @cache.clear
      @cache.silence!
      @cache.logger = ActiveSupport::Logger.new("/dev/null")
  ...
  class NullStoreTest < ::NullStoreTest
    def setup
      @secret = SecureRandom.hex(64)
      @cache = ActiveSupport::Cache.lookup_store(:null_store, encrypt: true, encryption_key: @secret)
    end
  end


  class CacheEntryTest < ::CacheEntryTest
    def setup
      @secret = SecureRandom.hex(64)
    ...
    def test_expired
      entry = ActiveSupport::Cache::Entry.new("value", encrypt: true)
      assert !entry.expired?, 'entry not expired'
      entry = ActiveSupport::Cache::Entry.new("value", :expires_in => 60, encrypt: true)
      assert !entry.expired?, 'entry not expired'
      time = Time.now + 61
      Time.stubs(:now).returns(time)
      assert entry.expired?, 'entry is expired'
    ...
    def test_non_compress_values
      ...
      assert_equal value, entry.value
      assert(value.bytesize <= entry.size, 'value.bytesize is larger than entry.size')
    end
  end
end
